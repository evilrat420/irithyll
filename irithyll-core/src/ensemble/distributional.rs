...
impl DistributionalSGBT {
    ...
    pub fn new(config: SGBTConfig) -> Self {
        let leaf_decay_alpha = config
            .leaf_half_life
            .map(|hl| crate::math::exp(-(crate::math::ln(2.0_f64)) / hl as f64));
    ...
    pub fn train_one(&mut self, sample: &impl Observation) {
        ...
        if !self.base_initialized {
            ...
            if self.initial_targets.len() >= self.initial_target_count {
                ...
                self.scale_base = crate::math::ln(initial_std);
    ...
    fn train_one_empirical(&mut self, target: f64, features: &[f64]) {
        ...
        let empirical_sigma = crate::math::sqrt(self.ewma_sq_err).max(1e-8);
        ...
        let sigma_ratio = if self.uncertainty_modulated_lr {
            ...
            let k_d = if self.rolling_sigma_mean > 1e-12 {
                crate::math::abs(self.sigma_velocity) / self.rolling_sigma_mean
            } else {
                0.0
            };
    ...
    pub fn predict(&self, features: &[f64]) -> GaussianPrediction {
        ...
        let mu = if let Some(ref cache) = self.packed_cache {
            ...
            match crate::EnsembleView::from_bytes(&cache.bytes) {
                Ok(view) => {
    ...
    pub fn empirical_sigma(&self) -> f64 {
        crate::math::sqrt(self.ewma_sq_err)
    }
    ...
    fn location_gradient(&self, mu: f64, target: f64) -> (f64, f64) {
        if let Some(k) = self.config.huber_k {
            let delta = k * crate::math::sqrt(self.ewma_sq_err).max(1e-8);
            let residual = mu - target;
            if crate::math::abs(residual) <= delta {
                (residual, 1.0)
            } else {
    ...
    pub fn ensemble_grad_std(&self) -> f64 {
        ...
        crate::math::fmax(
            crate::math::sqrt(self.ensemble_grad_m2 / (self.ensemble_grad_count - 1) as f64),
            0.0,
        )
    }
    ...
    ///
    /// In `irithyll-core` this is a no-op because `export_embedded` is not
    /// available. The full `irithyll` crate overrides this via its re-export
    /// layer to populate the cache with packed f32 bytes.
    fn refresh_packed_cache(&mut self) {
        // export_distributional_packed lives in irithyll (not irithyll-core),
        // so packed cache is not populated in core-only builds.
        // The full irithyll crate hooks into this via its own DistributionalSGBT
        // re-export which adds the packed cache logic.
    }
    ...
    pub fn diagnostics(&self) -> ModelDiagnostics {
            ...
            for step in steps {
                ...
                let leaf_weight_stats = if leaf_values.is_empty() {
                    ...
                    let var: f64 = leaf_values
                        .iter()
                        .map(|v| crate::math::powi(v - mean, 2))
                        .sum::<f64>()
        ...
        ModelDiagnostics {
            ...
            location_base: self.location_base,
            scale_base: self.scale_base,
            empirical_sigma: crate::math::sqrt(self.ewma_sq_err),
            scale_mode: self.scale_mode,
            scale_trees_active,
            auto_bandwidths: self.auto_bandwidths.clone(),
            ensemble_grad_mean: self.ensemble_grad_mean,
        ...
        let (sb, scale) = match self.scale_mode {
            ScaleMode::Empirical => {
                ...
                (
                    ...
                    vec![0.0; self.location_steps.len()],
                )
            }
            ScaleMode::TreeChain => {
                let s: Vec<f64> = self
...
mod tests {
    ...
    #[test]
    #[cfg(feature = "_packed_cache_tests_disabled")]
